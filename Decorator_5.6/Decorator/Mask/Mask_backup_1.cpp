...
//    cv::Mat alphaPattern;
//    cv::cvtColor(pattern, alphaPattern, CV_BGR2GRAY);
//    cv::threshold(alphaPattern, alphaPattern, 0, 255, cv::THRESH_BINARY);
...
//            cv::Mat m = sub_mask.clone();
//            alphaPattern(cv::Rect(0, 0, rect.width, rect.height)).copyTo(m,sub_mask);
			pattern(cv::Rect(0, 0, rect.width, rect.height)).copyTo(sub_dst, sub_mask);
        }
	}
/*
    cv::Mat gray;
    cv::cvtColor(imgSrc, gray, CV_BGRA2GRAY);
	cv::Mat planes[] = {gray, gray, gray};
    cv::Mat pl = cv::Mat::zeros(imgSrc.size(), CV_8UC3);
    cv::merge(planes, 3, pl);
    cv::add(0.3*pl, 0.7*imgDst, imgDst, mask);
    pl.release();
    gray.release();
*/
    cv::Mat gray;
    cv::cvtColor(imgSrc, gray, CV_BGRA2GRAY);
	cv::Mat planes[] = {cv::Mat::zeros(imgDst.size(), CV_8UC1),
		cv::Mat::zeros(imgDst.size(), CV_8UC1),  cv::Mat::zeros(imgDst.size(), CV_8UC1)};
    cv::Mat pl;
    cv::cvtColor(imgDst, pl, CV_BGR2HSV);
    cv::split(pl, planes);    
    cv::add(0.5*gray, 0.5*planes[2], gray, mask);
/*
    if ((referenceColor == -300)||(m_index==1))
       referenceColor = calculateColorInMask(gray, mask);
    int reference = referenceColor;
    int replace = calculateColorInMask(planes[2], mask);
    int plus = replace-reference;
    gray += plus;
*/
    double minVal1, maxVal1, minVal2, maxVal2;
    cv::minMaxLoc(planes[2], &minVal1, &maxVal1, NULL, NULL, mask);
    cv::minMaxLoc(gray, &minVal2, &maxVal2, NULL, NULL, mask);
    double compressRatio = (maxVal1-minVal1)/(maxVal2 - minVal2);
    double beta =(-minVal2+minVal1)*compressRatio;
    gray.convertTo(gray, CV_8U, compressRatio, beta);
    
    
    gray.copyTo(planes[2], mask);                                                                   
    cv::merge(planes, 3, pl);
    cv::cvtColor(pl, imgDst, CV_HSV2BGR);
    gray.release();
    pl.release();
    planes[0].release();
    planes[1].release();
