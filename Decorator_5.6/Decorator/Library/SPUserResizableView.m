...
@interface SPUGripViewBorderView : UIView{
}
@end

@implementation SPUGripViewBorderView
...
@synthesize isHideEditButton;
@synthesize isShowDeleteButton;
@synthesize preventChangeSizeRatio;

- (void)setupDefaultAttributes {
    borderView = [[SPUGripViewBorderView alloc] initWithFrame:CGRectInset(self.bounds, kSPUserResizableViewGlobalInset, kSPUserResizableViewGlobalInset)];
    ...
    if (!isHideEditButton) {
        CGRect upperLeft = CGRectMake(-10.0, -10.0, 40, 40);
        editButton = [[UIButton alloc] initWithFrame:upperLeft];
        [editButton setImage:[UIImage imageNamed:@"pencil"] forState:UIControlStateNormal];
        [editButton addTarget:self action:@selector(callEditAction:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:editButton];
        [editButton setHidden:YES];
    }
    
    if (isShowDeleteButton) {
        CGRect lowerRight = CGRectMake(self.bounds.size.width - 30, -10, 40, 40);
        deleteButton = [[UIButton alloc] initWithFrame:lowerRight];
        [deleteButton setImage:[UIImage imageNamed:@"ws_iconDeleteText"] forState:UIControlStateNormal];
        [deleteButton addTarget:self action:@selector(callDeleteAction:) forControlEvents:UIControlEventTouchUpInside];
        deleteButton.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleRightMargin;
        [self addSubview:deleteButton];
        [deleteButton setHidden:YES];
    }
}

- (id)initWithFrame:(CGRect)frame isHideEditControl:(BOOL)_hideEditControl{
    if ((self = [super initWithFrame:frame])) {
        isHideEditButton = _hideEditControl;
        [self setupDefaultAttributes];
    }
    return self;
}

- (id)initWithFrame:(CGRect)frame isHideEditControl:(BOOL)hideEditControl andIsShowDeleteControl:(BOOL)_showDeleteControl{
    if ((self = [super initWithFrame:frame])) {
        isHideEditButton = hideEditControl;
        isShowDeleteButton = _showDeleteControl;
        [self setupDefaultAttributes];
    }
    return self;
}
...
- (void)callDeleteAction:(id)sender{
    [delegate deleteContentView:self];
}

- (void)callEditAction:(id)sender{
    [delegate editContentView:self];
}

- (void)callRotateAction:(id)sender{
    [delegate rotateView:self];
}
...
- (void)setFrame:(CGRect)newFrame {
    ...
    deleteButton.frame = CGRectMake(newFrame.size.width - 30, -10, 40, 40);
    editButton.frame = CGRectMake(-10, -10, 40, 40);
}
...
- (void)resizeUsingTouchLocation:(CGPoint)touchPoint {
    ...
    if (preventChangeSizeRatio) {
        if (fabs(deltaH) > fabs(deltaW)) {
            deltaW = deltaH * (self.frame.size.width/self.frame.size.height);
        }
        else
            deltaH = deltaW * (self.frame.size.height/self.frame.size.width);
    }
...
//            newHeight = self.superview.bounds.size.height - newY;
    ...
    deleteButton.frame = CGRectMake(newWidth - 30, -10, 40, 40);
    editButton.frame = CGRectMake(-10, -10, 40, 40);
    touchStart = touchPoint;
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(didResizeEnd:)])
        [self.delegate didResizeEnd:self];
}
...
//        }
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(didMoveEnd:)])
        [self.delegate didMoveEnd:self];
}
