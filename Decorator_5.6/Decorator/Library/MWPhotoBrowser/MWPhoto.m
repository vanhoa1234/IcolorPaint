...
- (id)initWithPHAset:(PHAsset *)asset {
    if ((self = [super init])) {
        _asset = [asset copy];
    }
    return self;
...
- (void)loadUnderlyingImageAndNotify {
    ...
    if (self.underlyingImage) {
        ...
        if (_photoPath) {
            ...
            PHAsset *phasset = [PHAsset fetchAssetsWithALAssetURLs:@[_assetURL] options:nil].firstObject;
            ...
            MBProgressHUD *hud = [[MBProgressHUD alloc] initWithWindow:windows[0]];
            [hud show:YES];
            [manager requestImageForAsset:phasset targetSize:[UIScreen mainScreen].bounds.size contentMode:PHImageContentModeDefault options:option resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
                [hud hide:YES];
                    if (result != nil) {
            ...
            option.networkAccessAllowed = YES;
            option.synchronous = NO;
            MBProgressHUD *hud = [[MBProgressHUD alloc] initWithWindow:[[[UIApplication sharedApplication] windows] objectAtIndex:0]];
            [hud show:YES];
            [manager requestImageForAsset:_asset targetSize:[UIScreen mainScreen].bounds.size contentMode:PHImageContentModeDefault options:option resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
                [hud hide:YES];
                    if (result != nil) {
                        self.underlyingImage = result;
                        [self imageLoadingComplete];
...
- (void)unloadUnderlyingImage {
    ...
    [[SDWebImageManager sharedManager] cancelForDelegate:self];
    if (self.underlyingImage && (_photoPath || _photoURL || _assetURL)) {
        self.underlyingImage = nil;
    }
}
