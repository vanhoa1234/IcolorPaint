...
- (id)initWithDelegate:(id <MWPhotoBrowserDelegate>)delegate isShowBackground:(BOOL)_isShowBackground{
    if ((self = [self init])) {
        _delegate = delegate;
        isShowBackground = _isShowBackground;
	}
	return self;
}
...
//    _pagingScrollView.layer.borderColor = [UIColor redColor].CGColor;
//    _pagingScrollView.layer.borderWidth = 2.0f;
...
- (void)performLayout {
    ...
    if (!isShowBackground) {
        ...
        UIBarButtonItem *doneButton = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"選択", nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)] autorelease];
        // Set appearance
...
//        NSString *backButtonTitle = previousViewController.navigationItem.backBarButtonItem ? previousViewController.navigationItem.backBarButtonItem.title : previousViewController.title;
    UIBarButtonItem *newBackButton = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"back", nil) style:UIBarButtonItemStylePlain target:self action:@selector(closePhotoBrowser:)] autorelease];
        // Appearance
        if ([UIBarButtonItem respondsToSelector:@selector(appearance)]) {
...
//        previousViewController.navigationItem.backBarButtonItem = newBackButton;
    self.previousViewControllerBackButton = newBackButton;
...
- (void)closePhotoBrowser:(id)sender{
    [_delegate cancelPhotoBrowser];
}
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    _viewIsActive = YES;
    if (isShowBackground) {
        _background = [[UIImageView alloc] initWithImage:UIInterfaceOrientationIsLandscape([UIApplication sharedApplication].statusBarOrientation) ? [UIImage imageNamed:@"BG_02.jpg"] : [UIImage imageNamed:@"BG_04.jpg"]];
        _background.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (index < 0) index = 0;
	if (index > (int)[self numberOfPhotos] - 1) index = (int)[self numberOfPhotos] - 1;
...
- (void)updateNavigation {
	...
	if ([self numberOfPhotos] > 1) {
		self.title = [NSString stringWithFormat:@"%i %@ %i", (int)_currentPageIndex+1, NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), (int)[self numberOfPhotos]];
	} else {
		self.title = nil;
	}
...
- (void)doneButtonPressed:(id)sender {
    ...
    [_delegate selectedImageIndex:(int)_currentPageIndex];
}
